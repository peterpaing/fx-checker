...
export default function NavSelect() {
  ...
  return (
    <div
      className="relative w-6/7 mx-auto mt-4 md:hidden"
    ...
    >
      <button
        ...
        aria-haspopup="true"
        aria-label={`Select page, current page ${currentItem.label}`}
      ...
      >
        <div className="flex items-center gap-2">
          ...
          {currentItem.count !== undefined && (
            <span
              aria-label={`${currentItem.count} ${currentItem.label.toLowerCase()}`}
              className="bg-lime-900 px-2 py-1 rounded-full text-lime-400 text-xs"
            >
              {currentItem.count}
            </span>
          )}
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                focus-visible:ring-lime-400 focus-visible:ring-inset
                ${
                  isCurrent
